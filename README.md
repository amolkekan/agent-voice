...
Local **MCP** text-to-speech for AI agents on **macOS**: speak a **short summary** of a chat reply with the built-in **`say`** command. The full answer stays in the chat. No cloud TTS, no API key. Works with Cursor, Claude, ChatGPT, Gemini, VS Code, and other MCP hosts.
